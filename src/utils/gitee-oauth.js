...
export async function getAccessToken(code) {
  try {
    const res = await axios.post(`${baseOauthUrl}/token?grant_type=authorization_code&code=${code}&client_id=${clientId}&redirect_uri=${redirectUri}`, { client_secret: clientSecret })
...
export async function getUserInfo(access_token) {
  try {
    const res = await axios.get(`https://gitee.com/api/v5/user?access_token=${access_token}`)
    const { avatar_url, email, id, name, login, html_url } = res
